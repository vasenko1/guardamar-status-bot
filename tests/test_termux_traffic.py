...
class TrafficTermuxTests(unittest.TestCase):
    def _install(self, initial):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                ["sh", str(ROOT / "termux" / "install-traffic-cron.sh")],
                text=True,
                capture_output=True,
                env=environment,
                check=False,
    ...
    def test_installer_is_hourly_idempotent_and_preserves_other_jobs(self):
        ...
        self.assertEqual(installed.count("37 * * * *"), 1)
        self.assertIn("# BEGIN guardamar-status traffic monitor", installed)
        self.assertNotIn("*/30", installed)
