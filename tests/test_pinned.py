...
class PinnedContentTests(unittest.TestCase):
    ...
    def test_fishing_card_is_compact_stable_reference(self):
        ...
        self.assertIn("только при отсутствии пользователей на пляже", message)
        self.assertIn("снасти нужно убрать", message)
        self.assertIn("100 м", message)
        self.assertIn("бализованных каналах", message)
        self.assertIn("Подводная морская рыбалка", message)
        self.assertIn("только при отсутствии купающихся", message)
        self.assertIn("от заката до восхода", message)
        self.assertIn("сигнальный буй обязателен", message)
        self.assertIn("портовых водах", message)
        self.assertIn("Нужна соответствующая лицензия", message)
        self.assertIn("Условия и оформление — Generalitat Valenciana", message)
