...
- A pre-morning `sync-pharmacy` command makes one bounded fetch of the
  ...
  stores only rows from Guardamar's complete published pharmacy service zone
  `61` for the next 45 days as a small normalized catalog
  (`state/pharmacy.json`): date, title-cased name, address and municipality,
  plus structured zero-padded shift endpoints. Filtering only by postal
  municipality is forbidden because the zone also assigns duties in San
  Fulgencio; a Guardamar reinforcement row must not hide the pharmacy that
  covers the full night. The raw workbook is never stored, and rows with
...
- The Morning Digest reads today's rows from the catalog only — no network
  at 07:30 — and renders at most two duties under a singular or plural
  pharmacy heading, 24-hour duty first. Each entry shows `name,
  municipality`, an exact duty statement and the standard Google-Maps
  address link. A same-day shift is `Дежурит с 09:00 до 22:00`; a shift
  crossing midnight carries both calendar dates; equal endpoints are
  explicitly a 24-hour duty with both dates. The wording describes the
  official duty period and never claims that the pharmacy itself closes. A
...
- Benefits: high-value daily information from the authoritative source at
  ...
  parsing; complete zone coverage even when the duty is outside Guardamar's
  municipal boundary.
