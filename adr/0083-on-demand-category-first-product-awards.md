...
## 27 September 2026 amendment: source admission and OCU physical-test gate

The five-category launch gate is now met, so implementation may start.

A source/category is eligible for the production registry only when the reviewed
evidence is primarily about product quality or a clearly defined championship
result at national, European or world level. Innovation-only awards and
unordered recognition seals may be used for discovery, but do not create an
ordered category candidate on their own.

OCU requires an additional evidence guard. OCU now explains that some products
which were not laboratory-tested can receive AI-derived quality evaluations.
For this feature, an OCU candidate is eligible only when the exact OCU product
or study evidence explicitly proves **Analizado en el laboratorio** (or an
equivalent physical-test statement). AI-derived OCU evaluations are never used
as award evidence.

Historical award results are immutable reviewed facts. The lightweight runtime
does not need to reconstruct a past ranking on every due run. It must instead:

1. validate the pinned authority/product page with small source-specific
   identity/evidence markers;
2. validate the exact current retailer product and current price;
3. fail closed if either contract has drifted.

This keeps the phone cost bounded while preserving the reviewed source
semantics. Adding a new source or a new edition remains a research/code change,
not autonomous production web search.
