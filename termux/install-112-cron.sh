...
MONITOR="$PROJECT_DIR/termux/check-112.sh"
BACKUP_DIR="$HOME/.cache/crontab"
CURRENT=$(mktemp)
UPDATED=$(mktemp)
ERRORS=$(mktemp)
BEGIN_MARKER='# BEGIN guardamar-status 112 monitor'
END_MARKER='# END guardamar-status 112 monitor'
...
' "$CURRENT"; then
    echo "ОШИБКА: повреждён служебный блок 112 monitor в crontab" >&2
    exit 1
fi

if [ ! -f "$BACKUP_DIR/crontab.before-112-monitor" ]; then
    cp "$CURRENT" "$BACKUP_DIR/crontab.before-112-monitor"
fi
...
echo "Мониторинг 112/Previfoc установлен: каждый час в :19"
